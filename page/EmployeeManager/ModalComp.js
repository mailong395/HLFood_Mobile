...
import { ACTIVE_EMP, ACTIVE_TABLE } from '../../config/config';
import { Button } from 'react-native-paper';
import { useSelector } from 'react-redux';

const ModalComp = ({ isShow, props, handleCloseModal, empSelected }) => {
  ...
  const checkStatus = (id) => {
    switch (id) {
      case 0:
        if (empSelected.job_title < userSelector.job_title) {
          return false;
        }
        return true;
      case 1:
        if (empSelected._id !== userSelector._id && empSelected.job_title > 0 && empSelected?.tables?.length === 0) {
          return true;
        }
        return false;
      case 2:
        if (empSelected.job_title === 3) {
          return true;
        }
        return false;
        ...
        <TouchableWithoutFeedback>
          <View style={styles.modalView}>
            ...
            <View style={styles.modalBody}>
              <FlatList data={ACTIVE_EMP} renderItem={renderItem} numColumns={2} showsVerticalScrollIndicator={false} />
            </View>
          </View>
        </TouchableWithoutFeedback>
      </Pressable>
